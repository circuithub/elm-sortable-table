...
simpleTheadHelp ( name, status, onClick1 ) =
    let
        content =
            case status of
                Unsortable ->
...
view (Config { toId, toMsg, columns, customizations }) state data =
    let
        ...
        filterSortedData ( sortIndex, data1 ) =
            if customizations.rowFilter sortIndex data1 then
                Just data1
            ...
            Keyed.node "tbody" customizations.tbodyAttrs <|
                List.map (viewRow toId columns customizations.rowAttrs) <|
                    List.filterMap filterSortedData <|
                        List.indexedMap (\i a -> ( i, a )) sortedData
